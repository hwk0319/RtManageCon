...
logFile=${dirName}/2.failover-otherNodeShudown_${logFileName}.log
...
exit 0
 ...
 then
 sh /root/return/task/commScript/callback/callLogManager.sh  "error" "${taskId}" "${stepId}"  'no instance is runing STOPED SUCCESS.' 
 sh /root/return/task/commScript/callback/callTaskManager.sh 2 "${taskId}" "${stepId}" 100 'no instance is runing STOPED SUCCESS.' 
