#!/bin/bash
#this script stop  local crs
#===========================log=========================
dirName=$(cd "$(dirname "$0")"; pwd)
logFileName=$(date +'%Y-%m-%d-%H-%M-%S')
logFile=${dirName}/stop2-stopcrs_${logFileName}.log
...
if [ $num != 0 ];
then
  sh /root/return/task/commScript/callback/callLogManager.sh "info" "${taskId}" "${stepId}"  "NODE $(hostName) CRS STOPING." 
  sh /root/return/task/commScript/callback/callTaskManager.sh 1 "${taskId}" "${stepId}" 10 "NODE $(hostName) CRS STOPING." 
crsctl sotp crs 1>./suc.log 2>./err.log
 if [ $? = 0 ]
 then
  sh /root/return/task/commScript/callback/callLogManager.sh "info" "${taskId}" "${stepId}"  "NODE $(hostName) STOPED SUCCESS." 
  sh /root/return/task/commScript/callback/callTaskManager.sh 2 "${taskId}" "${stepId}" 100 "NODE $(hostName) STOPED SUCCESS." 
 else
 GETLOG
 msgDtial=$errmsg
  sh /root/return/task/commScript/callback/callLogManager.sh "error" "${taskId}" "${stepId}" "${msgDtial}" 
  sh /root/return/task/commScript/callback/callTaskManager.sh 3 "${taskId}" "${stepId}" 100 "NODE $(hostName) STOPED FAILED." 
