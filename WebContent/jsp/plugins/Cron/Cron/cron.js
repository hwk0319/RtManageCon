...
cronConfigValue="";
function getCronFunctionValue()
{
	return cronConfigValue;
}
...
$(function() {
	...
	vals.change(function() {
		...
		//回填周期字段
		$("table #period").val(item.join(" "));
		$("table #cronExpress").val(item.join(" "));
		cronConfigValue = item.join(" ");
		$("#iframe_cron",window.parent.document).parent().find("table:eq(0) tbody #period").val(item.join(" "));
		$("#heathadd",parent.document).find("table:eq(0) tbody #cron").val(item.join(" "));
		$("#cyctime",window.parent.document).val(item.join(" "));
		$("#iframe_cron",window.parent.document).parent().find("table:eq(0) tbody #cronExpress").val(item.join(" "));
		//新建任务也页面/RtManageCon/WebContent/jsps/taskmanage/task/createTask.jsp
		$("#iframe_cron",window.parent.document).parent().parent().find("table #cronExpress").val(item.join(" "));
	});
