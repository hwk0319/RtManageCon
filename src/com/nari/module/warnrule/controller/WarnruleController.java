package com.nari.module.warnrule.controller;
...
import com.nari.module.warnrule.service.WarnruleService;
...
@Controller
@RequestMapping(value="warnruleCon")
public class WarnruleController extends BaseController {
	
	private Logger logger = Logger.getLogger(WarnruleController.class);
	
	@Resource(name="warnruleService")
	private WarnruleService service;
	@Resource(name="operationlogService")
	private OperationlogService operService;
	
	@ResponseBody
	@RequestMapping(value="/search")
	public List<WarnrulePo> search(WarnrulePo po, HttpServletRequest request) throws Exception{
		String inOrOut = Util.getInOrOut(request);
		//判断是否需要添加审计数据
		User userInfo = (User) request.getSession().getAttribute("user");
		boolean result = isAudit("告警规则管理","查询");
		if(result){
			generservice.insert_oprt_logs("配置管理-告警规则管理","查询",userInfo.getUsername(),"用户查询告警规则管理数据",userInfo.getIpaddr(),0, SysConstant.OPRTLOG_SECCUSS, SysConstant.BUSSIENCE_EVENT, SysConstant.LOGS_EVENTS,inOrOut);
		}
	...
	public List<WarnrulePo> searchTask(@Param("po") WarnrulePo po){
		List<WarnrulePo> list = null;
		try {
			list = service.searchTask(po);
		} catch (Exception e) {
			logger.error(e);
		}
		return list;
	...
	public Map<String,String> update(@Validated WarnrulePo po, BindingResult bindingResult, HttpServletRequest request, String oper) throws Exception{
		String inOrOut = Util.getInOrOut(request);
...
//		RsaDecryptTool rsaTool = new RsaDecryptTool();
		if(oper != null && oper.equals("add")){
			...
			sb.append(po.getIndextype_id()).append(po.getIndex_warn_id() == null ? "" : po.getIndex_warn_id()).append(po.getUpper_limit() == null ? "" : po.getUpper_limit()).append(po.getLower_limit() == null ? "" : po.getLower_limit());
			sb.append(po.getStd_value() == null ? "" : po.getStd_value()).append(po.getUid()).append(po.getLevel()).append(po.getType());
			...
			//判断是否需要添加审计数据
			boolean result = isAudit("告警规则管理","新增");
			if(result){
				generservice.insert_oprt_log("配置管理-告警规则","新增",userInfo.getUsername(),"新增了一条告警规则",userInfo.getIpaddr(),po.getId(),inOrOut);
			}
		}else if(oper != null && oper.equals("edit")){
			/*String mdate = rsaTool.decrypt(po.getmData());//解密得到MD5密文
			//对传输过来的数据进行MD5加密
			StringBuffer sb = new StringBuffer();
			sb.append(po.getId()).append(po.getIndextype_id()).append(po.getIndex_warn_id() == null ? "" : po.getIndex_warn_id()).append(po.getUpper_limit() == null ? "" : po.getUpper_limit()).append(po.getLower_limit() == null ? "" : po.getLower_limit());
			sb.append(po.getStd_value() == null ? "" : po.getStd_value()).append(po.getUid()).append(po.getLevel()).append(po.getType());
			String a = URLEncoder.encode(sb.toString(),"UTF-8");//中文转义
			String newMdata = MD5.GetMD5Code(a);
			if(!mdate.equals(newMdata)){
			...
			generservice.insert_oprt_log("配置管理-告警规则","编辑",userInfo.getUsername(),"修改了一条告警规则",userInfo.getIpaddr(),po.getId(),inOrOut);
		}else if(oper != null && oper.equals("del"))
		{
			po.setUpdate_by(userInfo.getId());
			res = service.delete(po);
			generservice.insert_oprt_log("配置管理-告警规则","删除",userInfo.getUsername(),"删除了一条告警规则",userInfo.getIpaddr(),po.getId(),inOrOut);
		}else
		{
			res = 0;
		}
